...
* **Methods**
  ...
  * **getAuthCodeLink()**
    ...
    This is a helper function that can be used to generate a link to the OAuth2 user authentication page. *(The scopes and the redirect URI have to be set before using this method.)*
  ...
  * **getAccess(code)**
    ...
    **Returns :**  `String` representation of the user's access key. *(This is not the user's OAuth Access token, rather it's a key generated by the library. It is recommended to save this key as a session variable)*
  ...
  * **getAccessObject(key)**
    ...
    ***Note :*** *The* [`Access`](#access) *object is the response received when requesting access token.*
  ...
  * **getAuthorizedUser(key)**
    ...
    **Note :** *This needs you to have the* `identify` *scope. Also if you have the* `email` *scope, then the returned user will also have two extra fields -* `email` *and* `emailVerified`.
