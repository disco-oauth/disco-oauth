...
module.exports = class Client {
  ...
  setScopes(...scopes) {
    if (scopes.length < 1) throw new Error('No scopes provided.');
      ...
      this.scopes = [];
      for (let i of scopes) {
        if (scopesList.includes(i.toString().toLowerCase())) {
          if (!this.scopes.includes(i.toString().toLowerCase())) this.scopes.push(i.toString().toLowerCase());
  ...
  get authCodeLink() {
    if (this.scopes.length > 0 && this.redirectURI !== ''){
      let state = uid(16);
      return {
        url: `https://discord.com/api/oauth2/authorize?response_type=code&client_id=${this._id}&scope=${this.scopes.join('%20')}&state=${state}&redirect_uri=${this.redirectURI}&prompt=none`,
        state
      };
    }
