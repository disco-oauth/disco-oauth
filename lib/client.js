...
const {
  User, Guilds, Connections, UserCollection, ConnectionsCollection, GuildsCollection, Collection
} = require('./types');
...
class Client {
   ...
   * @param {Scope | Array<Scope>} scopes
   */
  setScopes(...scopes) {
  ...
  get auth() {
    if (this.scopes.length > 0 && this.redirectURI !== '') {
      let state = uid(16);
      return {
        link: `https://discord.com/api/oauth2/authorize?response_type=code&client_id=${
          this._id
        }&scope=${this.scopes.join('%20')}&state=${state}&redirect_uri=${this.redirectURI}&prompt=none`,
        state
      };
    ...
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        if (response.statusCode === 200) {
          ...
          let key = jwt.sign(token, this._secret);
          if (typeof this.users === 'undefined') this.users = new UserCollection(this);
          if (typeof this.guilds === 'undefined') this.users = new GuildsCollection(this);
          if (typeof this.connections === 'undefined') this.users = new ConnectionsCollection(this);
