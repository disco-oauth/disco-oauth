...
module.exports = class Client {
  ...
  setScopes(...scopes) {
    ...
    if (Array.isArray(scopes[0])) scopes = scopes[0];
  ...
  get auth() {
    if (this.scopes.length > 0 && this.redirectURI !== ''){
      let state = uid(16);
      return {
        link: `https://discord.com/api/oauth2/authorize?response_type=code&client_id=${this._id}&scope=${this.scopes.join('%20')}&state=${state}&redirect_uri=${this.redirectURI}&prompt=none`,
        state
      };
    }
