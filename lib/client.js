...
module.exports = class Client {
  ...
  get authCodeLink() {
    if (this.scopes.length > 0 && this.redirectURI !== ''){
      let state = uid(16);
      return {
        url: `https://discordapp.com/oauth2/authorize?response_type=code&client_id=${this._id}&scope=${this.scopes.join('%20')}&state=${state}&redirect_uri=${this.redirectURI}&prompt=none`,
        state
      };
    }
