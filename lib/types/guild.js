 ...
 * @property {String} bannerHash The guild's banner hash.
...
class Guild {
   ...
   */
  constructor({ id, name, icon, banner, discovery_splash, features = [], owner = false, permissions = 0 }) {
    this._id = id;
    this._name = name;
    this._iconHash = icon;
    this._bannerHash = banner;
    this._discoverySplash = discovery_splash;
  ...
  /**
   * The hashed info about the guild's banner.
  ...
  /**
   * The hashed info about the guild's discovery.
   * @returns {Object.discovery}
   */
  get discoveryHash() {
  ...
  /**
   * Returns a url to the guild banner.
   *
   * @param {number} size The size of the banner in pixels. (Defaults to 512)
   * @returns {string}
   *
   * @example let myFavIcon = myFavUser.bannerUrl(); // Gets a normal 512x512px icon.
   * @example let myFavIcon = myFavUser.bannerUrl(1024); // Gets a 1024x1024px icon.
   */
  bannerUrl(size = 512) {
    return this.bannerHash
      ? `https://cdn.discordapp.com/banners/${this.id}/${this.bannerHash}.png?size=${size}`
      : 'https://i.imgur.com/LvroChs.png';
  }

  /**
   * Converts the guild into a JSON Object
   * @returns {{createdAt: Date, features: string[], isOwner: boolean, permissions: string[], createdTimestamp: number, name: string, iconHash: Object.icon, id: string, iconUrl: string, bannerUrl: string, discovery_splash: string}}}
   */
  toJSON() {
    const { id, name, createdAt, createdTimestamp, features, iconHash, bannerHash, discoveryHash, isOwner, permissions } = this;
