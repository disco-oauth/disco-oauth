...
class Guild {
   ...
   */
  constructor({ id, name, icon, banner, splash, features = [], owner = false, permissions = 0 }) {
    this._id = id;
    this._name = name;
    this._iconHash = icon;
    this._bannerHash = banner;
    this._splashHash = splash;
  ...
  /**
   * The hashed info about the guild's splash.
   * @returns {Object.splash}
   */
  get splashHash() {
    return this._splashHash;
  }
   ...
   * @returns {{createdAt: Date, features: string[], isOwner: boolean, permissions: string[], createdTimestamp: number, name: string, iconHash: Object.icon, id: string, iconUrl: string, bannerUrl: string, splasHash: Object.splash}}}
   */
  toJSON() {
    const { id, name, createdAt, createdTimestamp, features, iconHash, bannerHash, splashHash, isOwner, permissions } = this;
