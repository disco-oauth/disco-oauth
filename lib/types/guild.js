...
class Guild {
  ...
  /**
   * Converts the guild object into a readable JSON object.
   * @returns {{createdAt: Date, features: *[], isOwner: boolean, permissions: [], createdTimestamp: *, name, iconHash, id, iconUrl: string}}
   */
  toJSON() {
    let { id, name, createdAt, createdTimestamp, features, iconHash, isOwner, permissions } = this;
    let iconUrl = this.iconUrl();
    return { id, name, createdAt, createdTimestamp, features, iconHash, iconUrl, isOwner, permissions };
  }
}
...
class Guilds extends Map {
  ...
  constructor(guilds) {
    super();
    for (let g of guilds) this.set(g.id, new Guild(g));
