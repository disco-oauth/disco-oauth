...
class Guild {
   ...
   */
  constructor({ id, name, icon, features = [], large, owner = false, permissions = 0 }) {
    this._id = id;
    this._name = name;
    this._iconHash = icon;
   ...
   * @returns {{createdAt: Date, features: string[], isOwner: boolean, permissions: string[], createdTimestamp: number, name: string, iconHash: Object.icon, id: string, iconUrl: string}}}
   */
  toJSON() {
    const { id, name, createdAt, createdTimestamp, features,iconHash, isOwner, permissions, large } = this;
