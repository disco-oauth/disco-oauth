...
class User {
  ...
  get createdTimestamp() {
    return parseInt(BigInt('0b' + parseInt(this.#id).toString(2)) >> 22n) + 1420070400000;
  }
  get createdAt() {
  ...
  /**
   * Converts the user object into a readable JSON object.
   * @returns {{userFlags: [], avatarUrl: string, bot: boolean, createdTimestamp: *, emailId: undefined, locale, premiumType: (string), discriminator: number, emailVerified: undefined, createdAt: Date, avatarHash: null, isMFAEnabled, id, tag: string, username}}
   */
  toJSON() {
    let { id, username, discriminator, tag, emailId, emailVerified, createdAt, createdTimestamp, locale, isMFAEnabled, bot, avatarHash, premiumType, userFlags  } = this;
    let avatarUrl = this.avatarUrl();
    return {
      id, username, discriminator, tag, emailId, emailVerified, createdAt, createdTimestamp, locale, isMFAEnabled, bot, avatarHash, avatarUrl, premiumType, userFlags
    };
  }
}
