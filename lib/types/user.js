 ...
 */
  bannerUrl(size = 512) {
    return `https://cdn.discordapp.com/${this.bannerHash ? '' : 'embed/'}banners/${this.avatarHash ? `${this.id}/${this.bannerHash}` : this.discriminator % 5
    }.${this.bannerHash ? (this.bannerHash.startsWith('a_') ? 'gif' : 'png') : 'png'}?size=${size}`;
  }


  /**
   * Converts the user into a JSON object.
   * @returns {{userFlags: string[], avatarUrl: string, bot: boolean, createdTimestamp: number, emailId: string, locale: string, premiumType: string, discriminator: string, emailVerified: boolean, createdAt: Date, avatarHash: (string|null), bannerHash: (string|null), isMFAEnabled: boolean, id: string, tag: string, username: string}}
   */
  toJSON() {
    const {
      id,
